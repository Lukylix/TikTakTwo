...
function drawClearColumn(idx, idy, spaceAvailable) {
  ...
  game.ctx.fillRect(
    ...
    (spaceAvailable + 1) * game.cube.height - game.spacing
  );
}
async function drawDropingJeton(idx, idy, playerTurn) {
  ...
  for (posY = posYCurrent; posY <= posEnd; posY = posY + increment) {
    ...
    game.ctx.font = fontSelect(
      "solid",
      game.color.player[playerTurn ? 0 : 1],
      87
    );
...
function drawMessageCenter(message, color, size) {
  ...
  game.ctx.fillRect(
    0,
    game.bounding.height / 2 - size / 2 + (size * 0.1) / 2,
    game.bounding.width,
    size * 1.1
  );
...
function checkWin(nbForWin) {
  ...
  for (side = 0; side < 2; side++) {
    ...
    for (
      x = side == 0 ? 0 : 1;
      x < (side == 0 ? game.colmun : game.colmun - 1);
      x++
    ) {
      ...
      for (directionX = 0; directionX < 2; directionX++) {
        ...
        if (
          ...
          for (
            y = side == 0 ? 1 : game.row - 2;
            side == 0 ? y < game.row : y >= 0;
            side == 0 ? y++ : y--
          ) {
            ...
            if (
              game.states[side == 0 ? y - 1 : y + 1][
                directionX == 0 ? xcopy - 1 : xcopy + 1
              ] == game.states[y][xcopy] &&
              game.states[y][xcopy] == game.playerTurn
            ) {
              alignedCount++;
  ...
  for (y = 0; y < game.row; y++) {
    ...
    for (x = 1; x < game.colmun; x++) {
      if (
        game.states[y][x - 1] == game.states[y][x] &&
        game.states[y][x] == game.playerTurn
      ) {
        alignedCount++;
        if (alignedCount >= nbForWin) {
          return true;
  ...
  for (x = 0; x < game.colmun; x++) {
    ...
    for (y = 1; y < game.row; y++) {
      if (
        game.states[y - 1][x] == game.states[y][x] &&
        game.states[y][x] == game.playerTurn
      ) {
        alignedCount++;
        if (alignedCount >= nbForWin) {
...
function checkWinFromInput(idx, idy, nbForWin) {
  let startX = idx - (nbForWin - 1) > 0 ? idx - (nbForWin - 1) : 0;
  let endX =
    idx + (nbForWin - 1) < game.colmun ? idx + (nbForWin - 1) : game.colmun - 1;
  ...
  for (diag = 0; diag < 2; diag++) {
    ...
    for (
      x =
        startX +
        (idx - startX > (diag == 0 ? idy - startY : endY - idy)
          ? idx - startX - (diag == 0 ? idy - startY : endY - idy)
          : 0);
      x <= endX;
      x++
    ) {
      if (diag == 0 ? currentY > endY : currentY < startY) {
        break;
      }
      console.log(`Checking [${currentY}][${x}]`);
      if (game.states[currentY][x] == game.playerTurn) {
        alignedCount++;
        if (alignedCount >= nbForWin) {
          return true;
        }
